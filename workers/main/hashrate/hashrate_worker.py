...
print(f"[HASHRATE] Worker bound to RPC {RPC.info()}")

# JSONL Pfad
HASHRATE_FILE = "/raid/data/bitcoin_dashboard/metrics_history/hashrate/hashrate_history.jsonl"
os.makedirs(os.path.dirname(HASHRATE_FILE), exist_ok=True)
...
def last_entry_date():
    data = tail_jsonl(HASHRATE_FILE, 1)
    if not data:
        return None
...
def write_redis_from_jsonl():
    print("[HASHRATE] Updating Redis")

    r.set("CHART_BTC_HASHRATE_1y", json.dumps({
        "history": tail_jsonl(HASHRATE_FILE, 365)
    }))
    r.set("CHART_BTC_HASHRATE_5y", json.dumps({
        "history": tail_jsonl(HASHRATE_FILE, 365 * 5)
    }))
    r.set("CHART_BTC_HASHRATE_10y", json.dumps({
        "history": tail_jsonl(HASHRATE_FILE, 365 * 10)
    }))
    r.set("CHART_BTC_HASHRATE_ever", json.dumps({
        "history": tail_jsonl(HASHRATE_FILE, 365 * 50)
    }))
...
def write_new_entry(today):
    ...
    with open(HASHRATE_FILE, "a") as f:
        f.write(json.dumps(entry, separators=(",", ":")) + "\n")
        f.flush()
        os.fsync(f.fileno())

    print(f"[HASHRATE] Written {today}")
...
def hashrate_worker_loop():
    print("[HASHRATE] Worker started")
    ...
    while True:
        try:
            ...
            if now.hour < DAILY_RUN_HOUR_UTC:
                ...
                print(f"[HASHRATE] Waiting for 01:00 UTC, sleeping {sleep_s}s")
                time.sleep(sleep_s)
                continue
            ...
            sleep_s = seconds_until_next_run(DAILY_RUN_HOUR_UTC)
            print(f"[HASHRATE] No work, sleeping {sleep_s}s until next 01:00 UTC")
            time.sleep(sleep_s)
